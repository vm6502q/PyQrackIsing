...
from .kawasaki_chain_sampler import sample_measurement_kawasaki_sparse
...
def maxcut_tfim_sparse(
    ...
    if n_qubits < heuristic_threshold_sparse:
        best_solution, best_value = (
            exact_spin_glass(G_m.data, G_m.indptr, G_m.indices) if is_spin_glass else exact_maxcut(G_m.data, G_m.indptr, G_m.indices)
        )
    ...
    thinning = 1
    best_solution, best_value = sample_measurement_kawasaki_sparse(G_m.data, G_m.indptr, G_m.indices, shots, thread_count, cum_prob, repulsion_base, thinning, is_spin_glass)
