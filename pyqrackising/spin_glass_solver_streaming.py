...
def pick_gray_seeds(best_theta, thread_count, gray_seed_multiple, G_func, nodes, n, is_spin_glass):
    ...
    if is_spin_glass:
        for i in prange(seed_count):
            ...
            energies[i] = compute_energy_streaming(seed, G_func, nodes, n)
            seeds[i] = seed
    else:
        for i in prange(seed_count):
            seed = gray_mutation(i, best_theta)
            energies[i] = compute_cut_streaming(seed, G_func, nodes, n)
            seeds[i] = seed
    ...
    if is_spin_glass:
        for i in prange(thread_count):
            ...
            for curr_idx in range(thread_iterations):
                ...
                energy = compute_energy_streaming(r_iterator, G_func, nodes, n)
                if energy > energies[i]:
        ...
        for i in prange(thread_count):
            ...
            for curr_idx in range(thread_iterations):
                ...
                gray_code_next(r_iterator, npow - (curr_idx + 1))
                energy = compute_cut_streaming(r_iterator, G_func, nodes, n)
                if energy > energies[i]:
                    states[i], energies[i] = r_iterator.copy(), energy
