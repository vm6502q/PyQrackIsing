...
G = nx.petersen_graph()
best_solution_bit_string, best_cut_value, best_node_groups = maxcut_tfim(G, quality=4)
```
...
solution_bit_string, cut_value, node_groups, energy = spin_glass_solver(G, quality=4, correction_quality=2, best_guess=None)
...
circuit, path_length = tsp_symmetric(G, quality=0, correction_quality=2, is_3_opt=True, k_neighbors=20)

print(f"Node count: {n_nodes}")
print(f"Path: {circuit}")
print(f"Path length: {path_length}")
```
We only provide a solver for the symmetric version of the TSP (i.e., the distance from "A" to "B" is considered the same as from "B" to "A"). The path must be a complete circuit back to its origin.
